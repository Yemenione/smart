...
import WorkSection from '@/Components/old_components/sections/Work';
import { motion } from 'framer-motion';
import { useLanguage } from '@/context/LanguageContext';

import { ProjectData } from '@/lib/api';

export default function Index({ initialProjects = [] }: { initialProjects?: ProjectData[] }) {
    const { t } = useLanguage();
    const { siteName } = usePage().props as any;

    return (
        <div className="bg-background min-h-screen pt-20 overflow-hidden">
            <Head title={`${t.nav.work} | ${siteName}`} />
            <Navbar />
