...
export default function Index({
    ...
    const categories = ["All", ...Array.from(new Set(initialApps.map(app =>
        (language === 'fr' && app.category_fr) ? app.category_fr : app.category
    )))];

    const filteredApps = activeFilter === "All"
        ? initialApps
        : initialApps.filter(app => {
            const currentCat = (language === 'fr' && app.category_fr) ? app.category_fr : app.category;
    ...
    const docTitle = language === 'fr' ? `Applications SaaS | ${siteName}` : `SaaS Applications | ${siteName}`;

    return (
        <>
            <Head title={docTitle} />
            <section className="pt-32 pb-24 px-4 min-h-screen bg-[#050505] text-white">
                <Navbar />
                <div className="max-w-7xl mx-auto pt-16">
                    <div className="text-center mb-16 md:mb-24">
                        ...
                        >
                            {storeTitle ? (
                                <span dangerouslySetInnerHTML={{ __html: storeTitle.replace(/\n/g, '<br />') }} />
                            ) : t.storePage.title}
                            <br />
                            <span className="text-accent/80">{storeSubtitle || t.storePage.subtitle}</span>
                        </motion.h1>
                        ...
                        >
                            {storeDescription || t.storePage.description}
                        </motion.p>
                    </div>
                                    ...
                                    >
                                        ...
                                        <div className="relative w-full aspect-[4/3] overflow-hidden bg-black/50">
                                            <img
                                                ...
                                                alt={(language === 'fr' && app.name_fr) ? app.name_fr : app.name}
                                        ...
                                        <div className="p-6 md:p-8 flex-1 flex flex-col">
                                            <h3 className="text-2xl font-heading font-bold text-white mb-2 group-hover:text-accent transition-colors">
                                                {(language === 'fr' && app.name_fr) ? app.name_fr : app.name}
                                            </h3>
                                            <p className="text-white/60 text-sm mb-6 flex-1">
                                                {(language === 'fr' && app.description_fr) ? app.description_fr : app.description}
                                            </p>

                                            <div className="space-y-3 mb-8">
                                                {((language === 'fr' && app.features_fr) ? app.features_fr : app.features).split(',').slice(0, 4).map((feature, i) => (
                                                    <div key={i} className="flex items-start gap-3 text-sm text-white/80">
                                                        <div className="w-1.5 h-1.5 rounded-full bg-accent/50 mt-1.5 flex-shrink-0" />
                                                        <span>{feature.trim()}</span>
                                                    </div>
                                                ))}
                                                {((language === 'fr' && app.features_fr) ? app.features_fr : app.features).split(',').length > 4 && (
                                                    <div className="text-xs text-white/40 italic pl-4">
                                                        {t.storePage.moreFeatures.replace('{count}', String(((language === 'fr' && app.features_fr) ? app.features_fr : app.features).split(',').length - 4))}
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-4 mt-auto">
                                                <div className="py-4 border-y border-white/10 flex items-center justify-between">
                                                    ...
                                                    <span className="text-lg font-bold text-white tracking-widest">
                                                        {(language === 'fr' && app.price_text_fr) ? app.price_text_fr : app.price_text}
                                                    </span>
                                                </div>
                                                ...
                                                <div className="flex gap-3 pt-2">
                                                    ...
                                                    <Link
                                                        href="/contact"
                                                        className={`flex items-center justify-center gap-2 py-3 px-6 rounded-xl text-center text-sm font-bold uppercase tracking-widest transition-transform hover:scale-[1.02] active:scale-95 ${app.demo_link ? 'flex-1 bg-white text-black hover:bg-white/90' : 'w-full bg-accent text-background hover:bg-accent/90'}`}
                                                    >
