...
export default function AgencyIndex({ faqs }: { faqs: FAQData[] }) {
    const { t } = useLanguage();
    const [activeFaq, setActiveFaq] = useState<number | null>(null);

    return (
        <div className="bg-background min-h-screen selection:bg-accent/20">
            <Head title={t.agencyPage.title} />
            <Navbar />
            ...
            <main>
                ...
                <section className="w-full h-[60vh] md:h-screen relative overflow-hidden">
                    <img
                        ...
                        alt={`${t.agencyOverview.title} Vision`}
                        className="absolute inset-0 w-full h-full object-cover grayscale opacity-30 mix-blend-luminosity hover:grayscale-0 hover:opacity-80 transition-all duration-[3s]"
                    />
                </section>
