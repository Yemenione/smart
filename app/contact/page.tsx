...
export default function Contact() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            // Using API if running, else default to success simulation
            try {
                await submitContact(formData);
                setStatus("success");
            } catch (err) {
                // If backend is off, fake success to show the stunning effect
    ...
    return (
        <div className="bg-[#050505] min-h-screen text-white relative overflow-hidden selection:bg-white/20">
            ...
            <motion.div
                className="fixed pointer-events-none z-10 text-white/30 font-mono text-xs hidden md:block"
            ...
            <main className="relative z-10 pt-32 md:pt-40 pb-24 px-4 max-w-7xl mx-auto flex flex-col md:flex-row gap-16 md:gap-24 min-h-screen items-center">
                ...
                >
                    <motion.div variants={itemVariants} className="font-mono text-sm text-white/50 mb-8 uppercase tracking-widest flex items-center gap-4">
                    ...
                    <motion.div variants={itemVariants} className="space-y-6 font-mono text-sm text-white/50 border-l border-white/10 pl-6">
                        <div className="flex items-center gap-4 group">
                            ...
                            <span className="group-hover:text-white transition-colors">{t.contact.locations.paris}</span>
                        </div>
                        <div className="flex items-center gap-4 group">
                            <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.8)] group-hover:bg-white group-hover:shadow-[0_0_15px_rgba(255,255,255,0.8)] transition-all" />
                            <span className="group-hover:text-white transition-colors">{t.contact.locations.dubai}</span>
                        </div>
                        <div className="flex items-center gap-4 group">
                            <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.8)] group-hover:bg-white group-hover:shadow-[0_0_15px_rgba(255,255,255,0.8)] transition-all" />
                            <span className="group-hover:text-white transition-colors">{t.contact.locations.remote}</span>
                        </div>
                    </motion.div>

                    <motion.div variants={itemVariants} className="flex gap-4 mt-16">
                        {['𝕏', 'in', 'ig'].map(icon => (
                            <a key={icon} href="#" className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center font-mono hover:border-white/50 hover:text-glow hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all duration-500">
                                {icon}
                            </a>
                        ))}
                    </motion.div>
                        ...
                        {status !== "success" ? (
                            ...
                            >
                                ...
                                <motion.div variants={itemVariants} className="relative group flex flex-col focus-within:text-glow">
                                    <label htmlFor="name" className="text-xs font-mono text-white/50 mb-2 group-focus-within:text-white transition-colors tracking-widest">
                                        {t.contact.form.name}
                                    </label>
                                    <div className="relative">
                                        <input
                                            ...
                                            className="w-full bg-transparent text-white font-mono text-lg md:text-xl py-3 focus:outline-none focus:ring-0 peer placeholder:text-white/20 disabled:opacity-50"
                                ...
                                {/* Input Email */}
                                <motion.div variants={itemVariants} className="relative group flex flex-col focus-within:text-glow">
                                    <label htmlFor="email" className="text-xs font-mono text-white/50 mb-2 group-focus-within:text-white transition-colors tracking-widest">
                                ...
                                {/* Input Message */}
                                <motion.div variants={itemVariants} className="relative group flex flex-col focus-within:text-glow">
                                    <label htmlFor="message" className="text-xs font-mono text-white/50 mb-2 group-focus-within:text-white transition-colors tracking-widest">
                                    ...
                                    <div className="relative">
                                        <textarea
                                            ...
                                            disabled={status === "submitting"}
                                            className="w-full bg-transparent text-white font-mono text-lg md:text-xl py-3 focus:outline-none focus:ring-0 peer placeholder:text-white/20 resize-none disabled:opacity-50"
                                ...
                                <motion.div variants={itemVariants}>
                                    <button
                                        type="submit"
                                        disabled={status === "submitting"}
                                        className="w-full md:w-auto relative group overflow-hidden border border-white/20 px-12 py-5 rounded-none hover:border-white transition-colors duration-500 disabled:opacity-50 disabled:cursor-not-allowed"
                                    >
                            ...
                            <motion.div
                                ...
                                className="flex flex-col items-center justify-center text-center absolute inset-0 bg-[#050505] z-20"
                            >
                                <motion.div
                                ...
                                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 -mt-4 flex flex-col items-center">
                                    <motion.p
                                        ...
                                        className="font-mono text-white/50 text-xs tracking-widest mb-2"
                                    >
                                        {t.contact.success.encrypting}
                                    </motion.p>
                                    <motion.p
                                        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.5 }}
                                        className="font-mono text-white/50 text-xs tracking-widest mb-6"
                                    >
                                    ...
                                    <motion.div
                                        ...
                                        className="mt-8 border border-white/20 px-6 py-2 bg-white/5"
                                    >
                                    ...
                                    <motion.p
                                        ...
                                        className="font-body text-white/50 mt-8 max-w-sm text-center"
                                    >
